...
Comparative evaluation of M-Flow's Episodic Retriever against Cognee's Graph Completion and Graphiti's Temporal Context Graph on the Evolving Events benchmark.

## Benchmarks

| Benchmark | Questions | Source | Task Type |
|-----------|-----------|--------|-----------|
| Evolving Events | 100 | Custom enterprise knowledge corpus (159 chunks) | Long-term cross-chunk fuzzy & multi-hop reasoning |

### Dataset

The Evolving Events corpus and question-answer pairs were created by the M-Flow team to test cross-chunk reasoning capabilities that existing public benchmarks (e.g., HotPotQA) do not specifically target. The corpus simulates enterprise knowledge that evolves over time across multiple documents. Questions require combining information from multiple chunks and handling fuzzy/approximate matching. The dataset was created before system-specific tuning and is provided in `data/` for full transparency and independent verification.
